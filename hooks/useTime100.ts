...
import { defaultPreferences, seedProjects, seedTasks } from "@/lib/seed";
import type { Priority, Project, Task, TaskStatus, UserPreferences } from "@/types";

const TASKS_KEY = "time100-tasks-v1";
const PROJECTS_KEY = "time100-projects-v1";
...
export function useTime100() {
  ...
  useEffect(() => {
    if (ready) localStorage.setItem(SETTINGS_KEY, JSON.stringify(preferences));
  }, [ready, preferences]);

  function addTask(input: Omit<Task, "id" | "createdAt" | "order" | "actualHours">) {
    const order = tasks.filter((task) => task.status === input.status).length;
    setTasks((current) => [
      ...current,
      { ...input, id: crypto.randomUUID(), createdAt: new Date().toISOString(), order, actualHours: 0 },
    ]);
  }
  ...
  function sortedByStatus(status: TaskStatus) {
    return tasks
      ...
      .sort((a, b) => priorityWeight[a.priority] - priorityWeight[b.priority] || a.order - b.order);
  }
  ...
  const projectProgress = useMemo(() => {
    return projects.map((project) => {
      ...
      return { project, totalHours, completedHours, actualHours, progress, taskCount: projectTasks.length };
    });
  }, [projects, tasks]);
